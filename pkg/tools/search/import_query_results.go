...
func validateGeneratedConfiguration(configuration string, candidates []importCandidate) error {
	...
	if len(actual.Resources) != len(candidates) {
		return fmt.Errorf("contains %d resource blocks; expected exactly %d", len(actual.Resources), len(candidates))
	}
...
func parseTerraformConfigurationShape(configuration, filename string) (terraformConfigurationShape, error) {
	...
	if len(body.Attributes) != 0 {
		return terraformConfigurationShape{}, fmt.Errorf("top-level attributes are not permitted; only resource and import blocks are allowed")
	}
	...
	for _, block := range body.Blocks {
		...
		case "resource":
			...
			if err := validateGeneratedResourceBlock(block); err != nil {
		...
		default:
			return terraformConfigurationShape{}, fmt.Errorf("top-level %q blocks are not permitted; only resource and import blocks are allowed", block.Type)
		}
	}
	return shape, nil
}

func validateGeneratedResourceBlock(block *hclsyntax.Block) error {
	for _, nested := range block.Body.Blocks {
		if nested.Type == "provisioner" || nested.Type == "connection" {
			return fmt.Errorf("%s blocks are not permitted in generated resources", nested.Type)
		}
		if nested.Type == "dynamic" && len(nested.Labels) == 1 && (nested.Labels[0] == "provisioner" || nested.Labels[0] == "connection") {
			return fmt.Errorf("dynamic %s blocks are not permitted in generated resources", nested.Labels[0])
		}
	}

	var unsafeFunction string
	diagnostics := hclsyntax.VisitAll(block.Body, func(node hclsyntax.Node) hcl.Diagnostics {
		call, ok := node.(*hclsyntax.FunctionCallExpr)
		if !ok {
			return nil
		}
		switch strings.ToLower(call.Name) {
		case "file", "filebase64", "fileexists", "fileset", "pathexpand", "templatefile":
			unsafeFunction = call.Name
		}
		return nil
	})
	if diagnostics.HasErrors() {
		return fmt.Errorf("failed to inspect generated resource: %s", diagnostics.Error())
	}
	if unsafeFunction != "" {
		return fmt.Errorf("filesystem-reading function %s is not permitted in generated resources", unsafeFunction)
	}
	return nil
}
...
func parseImportBinding(block *hclsyntax.Block) (importBinding, error) {
	if len(block.Body.Blocks) != 0 {
		return importBinding{}, fmt.Errorf("import blocks cannot contain nested blocks")
	}
	for name := range block.Body.Attributes {
		if name != "to" && name != "id" && name != "identity" {
			return importBinding{}, fmt.Errorf("import block contains unsupported attribute %q", name)
		}
	}
