...
func TestValidateGeneratedConfiguration(t *testing.T) {
	...
	t.Run("rejects unsupported top-level blocks", func(t *testing.T) {
		for _, block := range []string{
			`data "external" "exploit" { program = ["sh", "-c", "id"] }`,
			`module "untrusted" { source = "example/untrusted/test" }`,
			`provider "external" {}`,
		} {
			configuration := "resource \"test_resource\" \"example\" {}\nimport {\n  to = test_resource.example\n  id = \"expected\"\n}\n" + block
			assert.ErrorContains(t, validateGeneratedConfiguration(configuration, candidates), "only resource and import blocks are allowed")
		}
	})

	t.Run("rejects extra resources", func(t *testing.T) {
		configuration := "resource \"test_resource\" \"example\" {}\nresource \"test_resource\" \"extra\" {}\nimport {\n  to = test_resource.example\n  id = \"expected\"\n}"
		assert.ErrorContains(t, validateGeneratedConfiguration(configuration, candidates), "contains 2 resource blocks; expected exactly 1")
	})

	t.Run("rejects provisioners", func(t *testing.T) {
		configuration := `
...
import {
  ...
  id = "expected"
		...
		assert.ErrorContains(t, validateGeneratedConfiguration(configuration, candidates), "provisioner blocks are not permitted")
	})

	t.Run("rejects filesystem-reading functions", func(t *testing.T) {
		configuration := `
resource "test_resource" "example" {
...
import {
  ...
  id = "expected"
		...
		assert.ErrorContains(t, validateGeneratedConfiguration(configuration, candidates), "filesystem-reading function file is not permitted")
	})
}
