...
func TestSensitiveAttrPatternCompilesCaseInsensitive(t *testing.T) {
	t.Setenv(sensitiveAttrPatternEnv, "")
	pattern, err := sensitiveAttrPattern()
	if err != nil || pattern != nil {
		t.Fatalf("expected (nil, nil) when unset, got (%v, %v)", pattern, err)
	}

	t.Setenv(sensitiveAttrPatternEnv, "(secret|password)")
	pattern, err = sensitiveAttrPattern()
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !pattern.MatchString("DB_PASSWORD") {
		t.Errorf("expected pattern to match case-insensitively, got no match for DB_PASSWORD")
	}

	t.Setenv(sensitiveAttrPatternEnv, "(unterminated")
	if _, err := sensitiveAttrPattern(); err == nil {
		t.Errorf("expected an error for an invalid regex, got nil")
	}
}
