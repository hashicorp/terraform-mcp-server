...
`),
			...
			mcp.WithString("provider_namespace",
				mcp.Required(),
				mcp.Description("The namespace of the private provider. If registry_name is 'private', this is the namespace in your Terraform Cloud/Enterprise organization. If registry_name is 'public', this is the namespace in the public Terraform registry."),
			),
...
func getPrivateProviderDetailsHandler(ctx context.Context, request mcp.CallToolRequest, logger *log.Logger) (*mcp.CallToolResult, error) {
	...
	if err != nil {
		return nil, utils.LogAndReturnError(logger, "provider_namespace is required", err)
	}
	providerNamespace = strings.TrimSpace(providerNamespace)

	providerName, err := request.RequireString("provider_name")
	if err != nil {
		return nil, utils.LogAndReturnError(logger, "provider_name is required", err)
	}
