...
func TestGetTrimmedString(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "ValuePresent_NoWhitespace",
			args:         map[string]interface{}{"org": "my-org"},
			paramName:    "org",
			defaultValue: "",
			expected:     "my-org",
		},
		{
			name:         "ValuePresent_WithWhitespace",
			args:         map[string]interface{}{"org": "  my-org  "},
			paramName:    "org",
			defaultValue: "",
			expected:     "my-org",
		...
		{
			name:         "ValueAbsent_ReturnsDefault",
			args:         map[string]interface{}{},
			paramName:    "org",
			defaultValue: "default-org",
			expected:     "default-org",
		},
		{
			name:         "ValueAbsent_DefaultHasWhitespace",
			args:         map[string]interface{}{},
			paramName:    "org",
			defaultValue: "  default-org  ",
			expected:     "default-org",
		...
		{
			name:         "ValuePresent_OnlyWhitespace",
			args:         map[string]interface{}{"org": "   "},
			paramName:    "org",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			req := mockCallToolRequest(tc.args)
			result := GetTrimmedString(req, tc.paramName, tc.defaultValue)
