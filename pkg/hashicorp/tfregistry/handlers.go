...
func SearchModules(registryClient *http.Client, logger *log.Logger) (tool mcp.Tool, handler server.ToolHandlerFunc) {
	return mcp.NewTool("searchModules",
			...
			mcp.WithString("provider",
				mcp.Description("Terraform provider to limit the search on."),
				mcp.DefaultString(""),
			),
			mcp.WithString("namespace",
				mcp.Description("Terraform namespace to limit the search on."),
				mcp.DefaultString(""),
			),
			...
			providerValue := request.GetString("provider", "")
			namespaceValue := request.GetString("namespace", "")
			verifiedValue := request.GetBool("verified", false)

			var modulesData, errMsg string
			response, err := searchModules(registryClient, moduleQuery, currentOffsetValue, providerValue, namespaceValue, verifiedValue, logger)
			if err != nil {
